...
public abstract class IntrospectedTable {
    ...
    protected void calculateJavaClientAttributes() {
        ...
        if (stringHasValue(tableConfiguration.getMapperName())) {
            ...
            sb.append(fullyQualifiedTable.getDomainObjectName());
            //自定义后缀名称
            String xmlMapperSuffix = context.getJavaClientGeneratorConfiguration().getProperty("mapperSuffix");
            if (xmlMapperSuffix == null){
                sb.append("Mapper");
            }else {
                sb.append(xmlMapperSuffix);//$NON-NLS-1$
            }
        }
    ...
    protected String calculateMyBatis3XmlMapperFileName() {
        ...
        if (stringHasValue(tableConfiguration.getMapperName())) {
            ...
            //前缀
            String xmlMapperPrefix = context.getSqlMapGeneratorConfiguration().getProperty(PropertyRegistry.SQL_XML_MAPPER_PREFIX);
            //后缀
            String xmlMapperSuffix = context.getSqlMapGeneratorConfiguration().getProperty(PropertyRegistry.SQL_XML_MAPPER_SUFFIX);
            if (stringHasValue(xmlMapperPrefix) && stringHasValue(xmlMapperSuffix)){
                sb.append(fullyQualifiedTable.getDomainObjectName());
                sb.append("Mapper.xml"); //$NON-NLS-1$
            }else{
                if (stringHasValue(xmlMapperPrefix)){
                    sb.append(xmlMapperPrefix);
                }
                //转换成小写
                sb.append(fullyQualifiedTable.getDomainObjectName().toLowerCase());
                if (stringHasValue(xmlMapperSuffix)){
                    sb.append(xmlMapperSuffix);
                }
                sb.append(".xml");
            }
        }
